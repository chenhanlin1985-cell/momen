...
func _initialize() -> void:
	...
	var result: Dictionary = compiler.compile_writer_drafts(true)
	if not _to_bool(result.get("success", false)):
		for error_text: String in Array(result.get("errors", []), TYPE_STRING, "", null):
			push_error(error_text)
		quit(1)
		return
	print("Writer drafts exported to %s" % str(result.get("writer_draft_output_path", compiler.get_writer_draft_output_path())))
