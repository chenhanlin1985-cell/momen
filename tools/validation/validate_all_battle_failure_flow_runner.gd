...
func _initialize() -> void:
	...
	for battle_definition: Dictionary in content_repository.get_battle_definitions():
		...
		if options.is_empty():
			...
			continue
		if not _event_has_finish_run(options):
	...
	battle_ids.sort()
	print("all_battle_ids=%s" % ",".join(battle_ids))
	...
	print("validate_all_battle_failure_flow_runner: OK")
	quit()
